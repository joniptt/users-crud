...
export class EditClientesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.data){
      ...
      this.formCliente.get('responsiblePerson').setValue(this.data.responsiblePerson);
      this.internalResponsible = this.data.internalResponsible;
    }
  }
  ...
  save() {
    ...
    if (this.data) {
      ...
      this.genericService.patchClient(user.id,user).subscribe({
        next: (data) => {
          this.swal.success("Concluído","Cliente atualizado com sucesso.","Ok");
      ...
      this.genericService.insertClient(user).subscribe({
        next: (data) => {
          this.swal.success("Concluído","Cliente cadastrado com sucesso.","Ok");
          this.dialogRef.close();
